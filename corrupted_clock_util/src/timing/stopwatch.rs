    ...
    T: Default,
{
    ...
    last_resume_moment: UtcDateTime,
    is_paused: bool,
    ...
    T: TimeImpl + Default,
{
    fn passed(&self) -> ClockDuration {
        if self.is_paused {
            ...
            let delta_and_last_resumed = now - self.last_resume_moment;
    ...
    fn pause(&mut self) {
        ...
        let paused_at = self.time_impl.now();
        self.is_paused = true;
        let to_add = paused_at - self.last_resume_moment;
        self.passed_time_between_pauses =
    ...
    fn resume(&mut self) {
        ...
        let now = self.time_impl.now();
        self.paused_time = self.paused_time + (now - self.last_resume_moment).into();
        self.last_resume_moment = now;
    }

    fn reset(&mut self) {
        self.is_paused = false;
        self.last_resume_moment = self.time_impl.now();
    ...
    fn paused_time(&self) -> ClockDuration {
        let current_pause_delta = if self.is_paused() {
            (self.time_impl.now() - self.last_resume_moment).into()
        } else {
            Default::default()
        };
    ...
    T: TimeImpl + Default,
{
    pub fn new_with_impl(time_impl: T) -> Self {
        let start_moment = time_impl.now();
        let last_resume_moment = start_moment;
...
mod testing {
    ...
    fn start_watch_and_progress() {
        let (watcher, mut setter) = test_utils::new_mocked_stopwatch("2000-11-11 11:11:11");
