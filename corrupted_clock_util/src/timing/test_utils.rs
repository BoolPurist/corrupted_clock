...
use crate::timing::mocking_time::{MockNowTimeAccessor, MockTimeImpl};
...
pub fn add_to_now(setter: &mut MockNowTimeAccessor, delta: TimeDelta) {
    let later = setter.now().checked_add_signed(delta).unwrap();
    setter.set_now(later);
}

pub fn new_mocked_stopwatch(input: &str) -> (Stopwatch<MockTimeImpl>, MockNowTimeAccessor) {
    let (time_impl, accessor) = MockTimeImpl::new(new_utc_moment(input));
    (Stopwatch::new_with_impl(time_impl), accessor)
}
